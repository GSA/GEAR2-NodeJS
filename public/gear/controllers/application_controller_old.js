...
angular.module('dashboard').controller('ApplicationController', ['$route', '$scope', '$http', '$routeParams', '$filter', '$location', '$sce', '$window',
  'ApplicationsSrc', 'ApplicationsRetiredSrc', 'AppCapabilitiesSrc', 'AppTechnologiesSrc', 'AppPOCsSrc', 'ParentSystemsSrc', 'SysAppSrc', 'InterfacesSrc', 'AppInterfacesSrc', 'AppInterfacesv2Src', 'OrgInterfacesSrc',
...
function ($route, $scope, $http, $routeParams, $filter, $location, $sce, $window,
  ...
  $scope.createRetAppTable = function () {
    ...
    // Use the Organization 'query' method to send an appropriate GET request
    var apps = ApplicationsRetiredSrc.query();
       ...
       graph.links.sort(function(a,b){
         return (a.source <= b.source) && (a.target <= b.target)
			 ...
			 graph.nodes.forEach(function (d, i) {
			   ...
			   graph.nodes[i] = { "name": d.key, "id": d.value[0].id  };
			 });
