...
angular.module('dashboard').controller('StrategyController', ['$route', '$scope', '$http', '$routeParams', '$filter', '$location', '$sce', '$window', 'Goal',
  ...
  function($route, $scope, $http, $routeParams, $filter, $location, $sce, $window, Goal,
    ...
    $scope.createInvDetail = function() {
      ...
      var investment = InvestmentsSrc.query({
        id: $routeParams.id
      });
      investment.$promise.then(function() {
        $scope.investment = investment[0];
        $scope.pocs = InvestmentPOCsSrc.query({
          id: $routeParams.id
        });
        $scope.applications = InvestmentAppsSrc.query({
          id: $routeParams.id
        });
    ...
    $scope.createInvestmentBar = function() {
      ...
      invests.$promise.then(function() {
          ...
          $scope.investmentBarData = _.find(investdata, function(o) {
