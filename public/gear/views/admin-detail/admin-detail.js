...
.controller('AdminDetailCtrl', ['$scope', '$http', '$route', '$location', '$routeParams',
  function ($scope, $http, $route, $location, $routeParams) {
    $http({method: 'GET', url: '/ua'})
      .success(function (d) {
        $http({method: 'GET', url: '/api/v0.1/fisma/' + $routeParams.id})
          .success(function (f) {
            $scope.user = d.user;
            $scope.rows = f.rows;
            ...
            if (d.user && d.user.groups.search(access) < 0) {
              $('#edit-form')
                .tooltip({title: 'Access insufficient to edit', placement:'top', trigger: 'click'})
    ...
    $form.on('submit', function (ev) {
      ...
      var name = $('#f_name').val();
      var desc = $('#f_desc').val();
      $http({method: 'PUT', url: '/api/v0.1/fisma/' + $routeParams.id, data: {
        id: id,
        name: name,
        desc: desc
      }})
